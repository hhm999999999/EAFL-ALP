...
from ..data.split import split_time_iid
...
def depth_factor(H):
    return 1 if H<0.1 else(2 if H<0.2 else 3)
def flat_shared(m):
...
def run_iot1():
    cfg=MainCfg()
    X,y,n_cls,df=load_iot1(cfg.csv_iot1)
    base=LocalDS(X,y)
    subs=split_time_iid(base,5)
    Hs=[entropy(s) for s in subs]
    Hmin,Hmax=min(Hs),max(Hs)
    clients=[Client(i,subs[i],6,n_cls,Hmin,Hmax,cfg) for i in range(5)]
