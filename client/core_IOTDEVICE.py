import numpy as np,torch,torch.nn as nn,torch.optim as optim,base64,json,requests,time
from torch.utils.data import Dataset,DataLoader,Subset,random_split
from ..config import MainCfg
from ..data.loader_iot import load_client
...
def width_factor(H,Hmin,Hmax):
    return 0.4+(H-Hmin)/(Hmax-Hmin+1e-9)*(1.2)
def depth_factor(H):
    return 1 if H<0.05 else 2
def flat_shared(m):
    return np.concatenate([p.detach().cpu().numpy().ravel() for p in m.shared.parameters()])
...
def run_iot():
    cfg=MainCfg()
    X,y,n_cls,df=load_client(cfg.csv_proxy)
    ds=LocalDS(X,y)
    subs=split_time(ds,5)
